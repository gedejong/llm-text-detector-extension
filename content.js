...
(async function init() {
  ...
  const { llmSiteOverrides = {} } =
    await chrome.storage.local.get('llmSiteOverrides');
  ...
  const { llmDetectorEnabled, llmDetectorIgnoredHosts } =
  ...
  const isVisible = (el) => {
    ...
    return (
      st.display !== 'none' && st.visibility !== 'hidden' && el.offsetHeight > 0
    );
  };
  ...
  function highlightBlocks() {
    ...
    for (const el of candidates) {
      ...
      if (
        ...
        !el.innerText ||
        el.innerText.trim().length < MIN_LENGTH || // too small
      ...
      if (
        /(meta|timestamp|icon|badge|label|control|button)/i.test(
          el.className || '',
        )
      )
        continue;
      ...
      // site‑specific skips
      if (
        (SITE_CONFIG[host]?.excludeSelectors || []).some(
      ...
      if (ratio > siteThreshold) {
        ...
        const bad = [...new Set([...raw].filter((ch) => !isTypable(ch)))]
          .slice(0, 10)
          .map(
            (ch) =>
  ...
  const mo = new MutationObserver(() => {
    clearTimeout(mo._t);
    mo._t = setTimeout(highlightBlocks, 200);
  });
  ...
  /* ------------------------ fix‑button overlay ----------------------- */
  const REPLACEMENTS = {
    /* … full mapping (unchanged) … */
  };
  function replaceUntypeable(t) {
    return [...t]
      .map((ch) =>
        isTypable(ch)
          ? ch
          : (REPLACEMENTS[ch] ??
            ch.normalize('NFKD').replace(/[^\x00-\x7F]/g, '')),
      )
      .join('');
  }
    ...
    inputs.forEach((input) => {
      ...
      btn.onclick = () => {
        if (input.isContentEditable) {
          input.innerText = replaceUntypeable(input.innerText);
        } else {
          input.value = replaceUntypeable(input.value);
        }
